...
const Signup = (props) => {
    ...
    const loginWithGoogle = () => {
        signInWithGoogle( window.location = queryParams.callback? queryParams.callback: '/welcome' )
    }
    const onSubmit = async (e) => {
        ...
        if (state.email && state.password && state.firstname && state.lastname && state.phone) {
            ...
            if (!result.message) {
                ...
                window.location = queryParams.callback? queryParams.callback : '/welcome';
                    ...
                    <div className="row no-gutters">
                        ...
                        <div className="col-md-7">
                            <div className="card-body">
                                ...
                                <p className="login-card-description">Maak jouw account aan</p>
                                <form action="#!">
                                    {message.error &&
                                    ...
                                    <div className="form-group mb-4">
                                        <label htmlFor="password" class="sr-only">Wachtwoord</label>
                                        <input type="password" name="password" id="password" className="form-control" placeholder="***********" onChange={onChange} />
                                    </div>

                                    <div className="form-group mb-4">
                                        <label htmlFor="password" class="sr-only">Bevestig je wachtwoord</label>
                                        <input type="password" name="passwordConfirm" id="passwordConfirm" className="form-control" placeholder="***********" onChange={onChange} />
                                    </div>

                                    <input name="register" id="register" className="btn btn-block login-btn mb-4" type="button" value="Registreer" onClick={onSubmit} />
                                </form>

                                <div className='providers-login'>
                                    <h5 className='providers-login__title'>Of registreer met </h5>
                                    <button type="button" className="google-button" onClick={loginWithGoogle}>
                                        ...
                                        <span className="google-button__text">Registreer met Google</span>
                                    </button>
                                </div>

                                <p className="login-card-footer-text">Heb je al een account? <a href="/login" className="text-reset">Log hier in</a></p>
